// https://juejin.cn/post/6844904135603699719
// polyfill 解决兼容性问题 
...
class Exposure {
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) { // 进入视图触发
                    try {
                        ...
                        const eventParam = entry.target.attributes['monitor-exposure'].value;
                    
                        self.dataList.push({ eventParam: JSON.parse(eventParam) });
                        // 已经上报的节点、取消对该DOM的观察
    ...
    add(entry) {
        ...
        ele.forEach((ele, index) => {
            // console.log('ndex, ele',index, ele);
